    ...
    def forward(self, tensor_list):
        if isinstance(tensor_list, NestedTensor):
            ...
            for name, x in xs.items():
                ...
                # breakpoint()
                # resize the mask in each feature scale.
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if name == 'resnet50' and load_backbone == 'dino':
            checkpoint = torch.hub.load_state_dict_from_url('https://dl.fbaipublicfiles.com/dino/dino_resnet50_pretrain/dino_resnet50_pretrain.pth',map_location='cpu')
            # checkpoint = torch.hub.load('facebookresearch/dino:main','dino_resnet50')
            # breakpoint()
            state_dict = {k.replace("module.", ""): v for k, v in checkpoint.items()}
            backbone.load_state_dict(state_dict, strict=False)
